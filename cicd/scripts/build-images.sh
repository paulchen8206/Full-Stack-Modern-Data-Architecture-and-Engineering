...
docker build -t "${PRODUCER_IMAGE}" ./source-apps/ods_source
docker build -t "${PROCESSOR_IMAGE}" ./process-apps/ods_processor
...
docker build -t "${mdm_rds_pg_IMAGE}" ./process-apps/mdm-pyspark-sync
