...
CONNECTOR_FILES="${CONNECTOR_FILES:-s3-sales-order.json,s3-sales-order-line-item.json,s3-customer-sales.json,jdbc-sales-order-warehouse.json,jdbc-sales-order-line-item-warehouse.json,jdbc-customer-sales-warehouse.json}"
CONNECTOR_DELETE_LIST="${CONNECTOR_DELETE_LIST:-jdbc-sales-warehouse}"
...
if [ -n "$CONNECTOR_DELETE_LIST" ]; then
	OLD_IFS="$IFS"
	IFS=','
	set -- $CONNECTOR_DELETE_LIST
	IFS="$OLD_IFS"

	for connector_to_delete in "$@"; do
		if [ -z "$connector_to_delete" ]; then
			continue
		fi

		if curl -sS -f "$CONNECT_URL/connectors/$connector_to_delete" >/dev/null 2>&1; then
			echo "Deleting legacy connector: $connector_to_delete"
			curl -sS -f -X DELETE "$CONNECT_URL/connectors/$connector_to_delete" >/dev/null
		fi
	done
fi
