...
def write_postgres_table(df, jdbc_url: str, table: str, user: str, password: str) -> None:
    # Full-table overwrite keeps landing tables deterministic for each sync cycle.
    # This intentionally favors simplicity over incremental merge complexity.
    (
    ...
    while True:
        try:
            for mysql_table, pg_table in table_map.items():
                # Read source snapshot and immediately replace target snapshot to avoid
                # partial table skew across related MDM entities.
                df = read_mysql_table(
                    spark=spark,
